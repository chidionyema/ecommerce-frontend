...
// Extend Resource types to include additional ones.
type ResourceType =
  | "tutorial"
  | "ebook"
  | "template"
...
// getTypeColor mapping for all ResourceType values
const getTypeColor = (type: ResourceType, isPremium: boolean) => {
  if (isPremium)
    return {
      bg: cp.premium.main,
      light: alpha(cp.premium.light, 0.15),
      hover: cp.premium.dark,
      border: alpha(cp.premium.main, 0.2),
      text: cp.premium.dark
    };

  const typeMap: Record<ResourceType, { bg: string; light: string; hover: string; border: string; text: string }> = {
    tutorial: {
      bg: cp.primary.main,
      light: alpha(cp.primary.light, 0.15),
      hover: cp.primary.dark,
      border: alpha(cp.primary.main, 0.2),
      text: cp.primary.dark
    },
    ebook: {
      bg: "#7c3aed",
      light: alpha("#c4b5fd", 0.3),
      hover: "#6d28d9",
      border: alpha("#7c3aed", 0.2),
      text: "#5b21b6"
    },
    template: {
      bg: cp.secondary.main,
      light: alpha(cp.secondary.light, 0.15),
      hover: cp.secondary.dark,
      border: alpha(cp.secondary.main, 0.2),
      text: cp.secondary.dark
    },
    video: {
      bg: "#ec4899",
      light: alpha("#fbcfe8", 0.3),
      hover: "#db2777",
      border: alpha("#ec4899", 0.2),
      text: "#be185d"
    },
    course: {
      bg: cp.success.main,
      light: alpha(cp.success.light, 0.15),
      hover: cp.success.dark,
      border: alpha(cp.success.main, 0.2),
      text: cp.success.dark
    },
    article: {
      bg: "#14b8a6",
      light: alpha("#99f6e4", 0.3),
      hover: "#0d9488",
      border: alpha("#14b8a6", 0.2),
      text: "#0f766e"
    },
    guide: {
      bg: "#f97316",
      light: alpha("#fdba74", 0.3),
      hover: "#ea580c",
...
const ResourceCard: React.FC<ResourceCardProps> = ({ resource }) => {
  ...
  const color = getTypeColor(resource.type, resource.premium ?? false);

  // Enhanced resource with default values
  const enhancedResource: Resource = {
    ...resource,
    rating: resource.rating || (Math.floor(Math.random() * 10) + 40) / 10,
    reviewCount: resource.reviewCount || Math.floor(Math.random() * 500) + 50,
    lastUpdated:
      resource.lastUpdated ||
      "Updated " +
        ["1 week", "2 weeks", "1 month", "3 months"][Math.floor(Math.random() * 4)] +
      " ago",
  ...
  return (
    <Card
      ...
      whileHover={{
        y: -8,
        boxShadow: `0 16px 32px ${alpha(color.bg, 0.15)}, 0 4px 8px ${alpha(color.bg, 0.1)}`
      }}
    ...
    >
      ...
      {enhancedResource.authorName && (
        <Box sx={{ display: "flex", alignItems: "center", px: 2.5, pb: 1.5, gap: 1 }}>
          <Box
            sx={{
              width: 32,
              height: 32,
          ...
          <Box>
            <Typography variant="caption" sx={{ fontWeight: 600, fontSize: "0.75rem", display: "block", color: theme.palette.text.primary }}>
              {enhancedResource.authorName}
            </Typography>
      ...
      <Box sx={{ p: 2.5, pt: 2, flexGrow: 1, display: "flex", flexDirection: "column" }}>
        ...
        <Box sx={{ display: "flex", flexWrap: "wrap", gap: 0.75, mb: 1.5, minHeight: 30, overflow: "hidden" }}>
          {enhancedResource.tags.slice(0, 3).map((tag, i) => (
            <Chip
              ...
              sx={{
                ...
                borderRadius: "6px",
                "&:hover": { bgcolor: alpha(color.bg, 0.12), borderColor: alpha(color.bg, 0.3) }
              }}
            />
          ))}
          {enhancedResource.tags.length > 3 && (
            <Chip
              label={`+${enhancedResource.tags.length - 3} more`}
              size="small"
              sx={{
                height: 24,
                fontSize: "0.72rem",
          ...
          {enhancedResource.downloadable && (
            ...
            >
              ...
              <Typography variant="caption" sx={{ fontWeight: 600, fontSize: "0.7rem", color: cp.neutral[700] }}>
                {(enhancedResource.downloadCount ?? 0).toLocaleString()}
              </Typography>
            </Box>
          )}
        </Box>
        <Box sx={{ display: "flex", gap: 2, mb: 2 }}>
          {enhancedResource.type === "course" && (
            ...
            >
              <Box component="span" sx={{ width: 8, height: 8, borderRadius: "50%", bgcolor: cp.success.main }} />
          ...
          {enhancedResource.type === "video" && (
            ...
            >
              <Box component="span" sx={{ width: 8, height: 8, borderRadius: "50%", bgcolor: "#ec4899" }} />
              <Typography variant="caption" sx={{ fontWeight: 500 }}>
                HD quality
              </Typography>
            </Box>
          ...
          {enhancedResource.premium && (
            ...
            >
              <Box component="span" sx={{ width: 8, height: 8, borderRadius: "50%", bgcolor: cp.premium.main }} />
              <Typography variant="caption" sx={{ fontWeight: 500 }}>
                Premium content
              </Typography>
            </Box>
...
const CreateResourceModal: React.FC<CreateResourceModalProps> = ({ open, onClose }) => {
  const theme = useTheme();
  ...
  const fields: Field[] = [
    ...
    { xs: 12, label: "Description", name: "description", placeholder: "Describe your resource clearly", multiline: true, rows: 4, required: true },
    {
      ...
      name: "type",
      select: true,
      options: ["tutorial", "ebook", "template", "video", "course"].map(o => ({
        value: o,
  ...
  return (
    <Dialog
      ...
      maxWidth="md"
      fullWidth
      PaperProps={{ sx: { borderRadius: 2, boxShadow: "0 10px 40px rgba(0,0,0,0.1)" } }}
    >
      <DialogTitle
        sx={{
          pb: 1,
          ...
          fontSize: "1.5rem",
          color: theme.palette.text.primary,
          borderBottom: `1px solid ${cp.neutral[200]}`
        }}
      >
        Create New Resource
      </DialogTitle>
      ...
      <DialogActions sx={{ px: 3, pb: 3, pt: 1 }}>
        <Button
          ...
          sx={{
            ...
            px: 3,
            py: 1,
            borderColor: cp.neutral[300],
            color: cp.neutral[700],
            "&:hover": { borderColor: cp.neutral[400], bgcolor: cp.neutral[50] }
          }}
        >
          Cancel
        </Button>
...
const ResourcesPage: React.FC = () => {
  ...
  const resources = useMemo(
    () =>
      resourcesData.map((r, i) => ({
        ...
        authorName: r.authorName || (i % 4 === 0 ? "Community User" : undefined)
      })),
    []
  );
  ...
  const filtered = resources.filter(r =>
    (!searchQuery ||
      [r.title, r.description, ...r.tags].some(t =>
  ...
  const tags = useMemo(
    () => ["all", ...Array.from(new Set(resources.flatMap(r => r.tags))).slice(0, 5)],
    [resources]
  );
      ...
      <Box sx={{ maxWidth: 800, mx: "auto", textAlign: "center" }}>
        ...
        >
          <TextField
            ...
            InputProps={{
              ...
              sx: {
                ...
                borderRadius: { xs: 1.5, sm: "12px 0 0 12px" },
          ...
          <Button
            ...
            sx={{
              px: 3,
              borderRadius: { xs: 1.5, sm: "0 12px 12px 0" },
              height: { sm: 56 },
              fontWeight: 600,
              textTransform: "none",
              bgcolor: cp.premium.main,
              "&:hover": {
                bgcolor: cp.premium.dark,
      ...
      <Container maxWidth="xl" sx={{ pt: 3, pb: 8 }}>
        ...
        <Box
          ...
          sx={{
            ...
            gap: 2,
            borderRadius: 2,
            bgcolor: cp.neutral[50],
            border: `1px solid ${cp.neutral[200]}`,
            ...
            {tags.map((tag, i) => (
              <Chip
                ...
                sx={{
                  ...
                  height: 32,
                  borderRadius: 10,
                  bgcolor: activeCategory === tag ? cp.primary.main : cp.neutral[100],
                  color: activeCategory === tag ? "#fff" : cp.neutral[700],
                  border: activeCategory === tag ? "none" : `1px solid ${cp.neutral[200]}`,
                  px: 1.2,
          ...
          <Box sx={{ display: "flex", alignItems: "center", gap: 2, width: { xs: "100%", md: "auto" } }}>
            <Tabs
              ...
              sx={{
                ...
                "& .Mui-selected": {
                  color: activeTab === 0 ? cp.primary.main : cp.premium.main,
                  fontWeight: 600
                }
              }}
            >
            ...
            <Button
              ...
              sx={{
                ...
                boxShadow: `0 4px 12px ${alpha(cp.primary.main, 0.2)}`
              }}
            >
              Create Resource
            </Button>
        ...
        >
          ...
          {activeTab === 1 ? " (Premium only)" : ""}
          {activeCategory !== "all" ? ` with tag "${activeCategory}"` : ""}
            ...
            >
              ...
              <Button
                ...
                sx={{
                  ...
                  textTransform: "none",
                  borderRadius: 1.5,
                  borderColor: cp.primary.main,
                  color: cp.primary.main,
          ...
          >
            <Box maxWidth={460}>
              ...
              <Box sx={{ display: "flex", flexDirection: { xs: "column", sm: "row" }, gap: 2 }}>
                ...
                <Button
                  component={motion.div}
                  whileHover={{ scale: 1.04, borderColor: "#fff", backgroundColor: "rgba(255,255,255,0.1)" }}
                  whileTap={{ scale: 0.97 }}
                  variant="outlined"
            ...
            <Box
              component={motion.div}
              whileHover={{ boxShadow: "0 16px 48px rgba(0,0,0,0.2)", y: -4 }}
              sx={{
