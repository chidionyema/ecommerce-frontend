...
import { 
  ...
  Chip, Avatar, Rating, Tabs, Tab, alpha,
  Paper
} from "@mui/material";
import { 
  Verified, GitHub, LinkedIn, Star, LocationOn, CalendarToday
...
// Define the type for generated params by Next.js
// Using Promise type for both params and searchParams as required by your Next.js configuration
type PageProps = {
...
export default async function CreatorProfilePage({ 
  ...
  // Resolve the params promise to get the creatorId
  const { creatorId } = await params;
  
  // Resolve searchParams if needed (using empty object as fallback)
  ...
  return (
    <ConsistentPageLayout title={`${creator.name} - GluStack Creator`}>
      ...
      <Box sx={{ mb: 4 }}>
        ...
        {/* Static tabs for server component */}
        <Box sx={{ mb: 3 }}>
          <Tabs 
            value={0}
            ...
            <Grid item xs={12} sm={6} md={4} key={stack.id}>
              <Card sx={{ 
                ...
                <CardContent sx={{ flexGrow: 1, display: 'flex', flexDirection: 'column' }}>
                  ...
                  >
                    <Link href={`/marketplace/glustack/${stack.id}`}
                      style={{ 
                        textDecoration: 'none', 
                        color: 'inherit',
                      }}
