...
const TestimonialsSection = () => {
  ...
  // Simplified styling objects
  const sx = {
    section: { position: 'relative', py: 10, background: 'linear-gradient(180deg, #18407F 0%, #1A438A 100%)', overflow: 'hidden' },
    header: { mb: { xs: 5, md: 6 } },
    ...
    ctaTitle: { letterSpacing: '-0.01em', fontSize: '1.3rem', fontWeight: 600, mb: 1, align: 'center', color: theme.palette.primary.main },
    ctaSubtitle: { letterSpacing: '0.01em', maxWidth: '85%', mx: 'auto', fontSize: '0.95rem', mb: 4, align: 'center', color: theme.palette.text.secondary },
    ...
    primaryBtn: {
      px: 2.5, py: 0.8, textTransform: 'none', fontWeight: 500, fontSize: '0.85rem', borderRadius: 6,
      letterSpacing: '0.01em', boxShadow: `0 2px 8px ${alpha(theme.palette.primary.main, 0.25)}`,
      transition: 'all 0.2s ease', flexGrow: { xs: 1, sm: 0 }, width: { xs: '100%', sm: 'auto' },
      '&:hover': { transform: 'translateY(-1px)', boxShadow: `0 4px 12px ${alpha(theme.palette.primary.main, 0.3)}` },
    },
    secondaryBtn: {
      px: 2.5, py: 0.8, textTransform: 'none', fontWeight: 500, fontSize: '0.85rem', borderRadius: 6,
      borderWidth: 1, letterSpacing: '0.01em', transition: 'all 0.2s ease',
      flexGrow: { xs: 1, sm: 0 }, width: { xs: '100%', sm: 'auto' },
      '&:hover': { borderWidth: 1, transform: 'translateY(-1px)', backgroundColor: alpha(theme.palette.primary.main, 0.05) },
    }
  };
    ...
    <TechCard
      icon={
        <Avatar 
          ...
          sx={{ width: 70, height: 70, border: `2px solid ${theme.palette.primary.main}`,
      ...
      importance="primary"
      sx={{
        pt: 4.5, position: 'relative', overflow: 'visible',
        background: `linear-gradient(145deg, ${alpha('#1a56db', 0.12)}, ${alpha('#1a56db', 0.05)})`,
        border: `1px solid ${alpha('#4285f4', 0.12)}`, boxShadow: `0 4px 20px ${alpha('#000', 0.05)}`
      }}
    >
      <Chip 
        label={testimonial.projectType} size="small" variant="outlined"
        sx={{ position: 'absolute', top: 8, right: 8, backgroundColor: alpha(theme.palette.primary.main, 0.12),
              borderColor: alpha(theme.palette.primary.main, 0.15), color: theme.palette.primary.main, 
              fontWeight: 500, fontSize: '0.65rem', height: '20px', '& .MuiChip-label': { px: 1 } }} 
      />
      <Rating 
        value={testimonial.rating} readOnly 
      ...
      />
      <Typography 
        variant="body1" sx={{ fontWeight: 400, color: alpha('#fff', 0.95), mb: 2.5, fontStyle: 'italic',
                             lineHeight: 1.6, fontSize: '0.85rem', height: '6.8rem', overflow: 'hidden',
                             textOverflow: 'ellipsis', display: '-webkit-box', WebkitLineClamp: 5, 
                             WebkitBoxOrient: 'vertical', textAlign: 'center', letterSpacing: '0.01em' }}
      ...
      <Box sx={{ textAlign: 'center', mt: 'auto' }}>
        ...
        <Typography variant="caption" sx={{ color: alpha('#fff', 0.85), display: 'block', 
                                           fontWeight: 400, fontSize: '0.75rem', letterSpacing: '0.01em' }}>
          {testimonial.role}
        </Typography>
      </Box>
    </TechCard>
  ...
  const ElegantCheckmarkItem = ({ text }) => (
    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.2, mb: 1.5 }}>
      <Box sx={{ width: 16, height: 16, borderRadius: '50%', backgroundColor: theme.palette.primary.main,
                display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontSize: '0.7rem' }}>
        <FileText size={8} />
      </Box>
      <Typography color={theme.palette.text.primary} sx={{ fontSize: '0.9rem', letterSpacing: '0.015em' }}>{text}</Typography>
    </Box>
  );
          ...
          <motion.div variants={ANIMATIONS.item}>
            <TechCard title="Enterprise Proof Points" sx={sx.ctaCard}>
              ...
              <Box sx={sx.buttonContainer}>
                <Button 
                  variant="contained" color="primary" href="/case-studies" 
                  startIcon={<Download size={14} />} sx={sx.primaryBtn}
                >Download Case Studies</Button>
                
                <Button 
                  variant="outlined" color="primary" onClick={() => setShowAll(!showAll)} 
                  endIcon={showAll ? <ChevronUp size={14} /> : <ChevronDown size={14} />} sx={sx.secondaryBtn}
                >{showAll ? 'Show Fewer Stories' : 'View All Testimonials'}</Button>
              </Box>
            </TechCard>
          </motion.div>
        </motion.div>
