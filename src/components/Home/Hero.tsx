...
const FadeInView: React.FC<FadeInViewProps> = ({ children, delay = 0, once = false, componentStyle }) => (
  <motion.div initial="hidden" animate={!once ? "visible" : undefined} whileInView={once ? "visible" : undefined} viewport={once ? { once: true } : undefined} variants={fadeVariants} transition={{ delay }} style={{ width: "100%", ...componentStyle }} >
    {children}
  </motion.div>
);
        ...
        <ContentArea>
          <FadeInView componentStyle={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%' }}>
            ...
            >
              ...
              <OfferChip icon={<Clock size={18} strokeWidth={2} />} label="Limited Time: Free Strategy Sessions" />
              {CTAButtonGroup}
            </Stack>
          </FadeInView>
